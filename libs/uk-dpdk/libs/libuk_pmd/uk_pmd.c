				   ...
				   uint16_t nb_pkts)
{
	...
	if (unlikely(!vrtl_eth_dev->data->dev_link.link_status)) {
		...
		while (rx_count < nb_pkts) {
			rc = uk_netdev_rx_one(dev_private->netdev,
					rxq->queue_id, &nb[rx_count]);
			UK_ASSERT(nb[rx_count]);
			mbuf = nb[rx_count]->priv;
			//printf("%s: mbuf %p\n", __func__, mbuf);
			/**
			...
			mbuf->pkt_len = nb[rx_count]->len;
			mbuf->data_len = nb[rx_count]->len;
			bufs[rx_count] = mbuf;
            rx_count++;
				   ...
				   uint16_t nb_pkts)
{
	...
	if (!vrtl_eth_dev->data->dev_link.link_status) {
        ...
        /**
         * use tc to record the count and send one packet everytime.
        */
        int tc = count;
        while (tc)
        {
            rc = uk_netdev_tx_one(dev_private->netdev,txq->queue_id,&nb[0]);
            UK_ASSERT(rc>=0);
            tc--;
        }
