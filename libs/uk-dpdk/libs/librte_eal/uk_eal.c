...
int rte_eal_init(int argc, char **argv)
{
	...
	struct ukplat_lcpu_func loop_func = {eal_thread_loop, NULL};
	int idx[RTE_MAX_LCORE];
	void *stack[RTE_MAX_LCORE];
    // void *stack[] = {a->palloc(a, 8) + 8 * __PAGE_SIZE,
	// 		 a->palloc(a, 8) + 8 * __PAGE_SIZE};
	for (int i = 1; i < RTE_MAX_LCORE; i++) {
		idx[i] = i;
		stack[i] = a->palloc(a, 8) + 8 * __PAGE_SIZE;
	}
	unsigned int num = sizeof(idx) / sizeof(__lcpuidx);
	int r;
	r = ukplat_lcpu_start(idx, &num, stack, NULL, 0);
	if (unlikely(rc))
		uk_pr_err("Failed start: %d\n", r);
	r = ukplat_lcpu_wait(NULL, 0, 0);
	if (unlikely(rc))
	...
	ukplat_lcpu_run(idx, &num, &loop_func, 0);
